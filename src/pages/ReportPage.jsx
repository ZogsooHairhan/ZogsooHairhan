...
function ReportPage() {
  ...
  const [reportData, setReportData] = useState({ totalAmount: 0, orderCount: 0, cancelledCount: 0, topItems: [], compareText: '', comparePercent: 0, diffAmount: 0 });
  
  // 🔐 ШИНЭ: Хаалт хийсэн мэдээлэл хадгалах хувьсагч
  ...
  const fetchAdvancedReport = async (filter) => {
    ...
    try {
      ...
      if (filter === 'today') {
        ...
        // ✨ Касс хаалтын мэдээллийг татаж авах
      ...
      const { data: currentData } = await supabase.from('orders').select(`total_amount, status, order_items (quantity, price, menu_items (name))`).gte('created_at', startDate.toISOString()).lte('created_at', endDate.toISOString()).in('status', ['completed', 'cancelled']);
      ...
      setReportData({ totalAmount: total, orderCount: count, cancelledCount: cancelled, topItems: Object.entries(itemMap).sort((a, b) => b[1].qty - a[1].qty), compareText: filter === 'today' ? 'Өчигдөртэй харьцуулахад' : 'Өмнөх сартай харьцуулахад', comparePercent: percent.toFixed(1), diffAmount: diff });
    } catch (err) {
      console.error(err);
    } finally {
  ...
  if (!isAuthenticated) return (<div>Нэвтрэх хэсэг... (Товчилсон)</div>); // Дээр хуулсан тул энд алгаслаа
    ...
    <div style={{ padding: '20px', fontFamily: 'sans-serif', backgroundColor: '#f8fafc', minHeight: '100vh', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '3px solid #e2e8f0', paddingBottom: '15px', marginBottom: '25px', flexWrap: 'wrap', gap: '15px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={() => supabase.auth.signOut()} style={{ padding: '12px 20px', fontSize: '1rem', fontWeight: 'bold', border: 'none', borderRadius: '8px', cursor: 'pointer', backgroundColor: '#ef4444', color: 'white' }}>🚪 Гарах</button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        {/* ✨ ШИНЭ: Менежерт харагдах хаалтын мэдээлэл (Зөвхөн өнөөдөр дээр гарна) */}
        {reportFilter === 'today' && shiftClosureData && (
          <div style={{ backgroundColor: '#dcfce7', color: '#16a34a', padding: '20px', borderRadius: '16px', border: '2px solid #bbf7d0', display: 'flex', alignItems: 'center', gap: '15px', fontSize: '1.1rem', boxShadow: '0 4px 15px rgba(22, 163, 74, 0.1)' }}>
            ...
            <div>
              <strong style={{ display: 'block', fontSize: '1.2rem', marginBottom: '5px' }}>Өнөөдрийн орлого хаагдсан байна!</strong>
              Систем дэх орлого: <b>{shiftClosureData.expected_cash.toLocaleString()} ₮</b> &nbsp;|&nbsp; 
              Бэлнээр тоолсон: <b>{shiftClosureData.actual_cash.toLocaleString()} ₮</b>
          ...
          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 300px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
                <h3 style={{ margin: '0 0 10px 0', color: '#64748b', fontSize: '1.2rem' }}>{reportFilter === 'today' ? 'Өнөөдрийн орлого' : 'Энэ сарын орлого'}</h3>
